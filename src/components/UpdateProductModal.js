...
const UpdateProductModal = ({ 
  ...
  onClose, 
  restaurantId,
  productData, // Dados do produto a ser editado
  onProductUpdated 
}) => {
  ...
  // ✅ Preencher campos quando os dados do produto mudarem
  useEffect(() => {
    if (productData && visible) {
      console.log('📝 Carregando dados do produto para edição:', productData);
      
      setNome(productData.nome || '');
      setDescricao(productData.descricao || '');
      setPreco(productData.valor ? productData.valor.toString().replace('.', ',') : '');
      setTempoPreparo(productData.tempo_preparo ? productData.tempo_preparo.toString() : '30');
      
      // Configurar selos selecionados
      const selos = productData.selos || {};
      const selosAtivos = [];
      
      if (selos.vegano) selosAtivos.push('vegano');
      if (selos.sem_gluten) selosAtivos.push('sem_gluten');
      if (selos.sem_amendoim) selosAtivos.push('sem_amendoim');
      if (selos.sem_lactose) selosAtivos.push('sem_lactose');
      
      setSelectedSelos(selosAtivos);
    }
  }, [productData, visible]);
  ...
  const handleSalvarEdicao = async () => {
    ...
    if (!productData || !productData.produto_id) {
    ...
    try {
      ...
      console.log('📦 Dados do produto para atualização:', updatedProductData);

      // Atualizar produto via API
      const produtoAtualizado = await RestaurantService.updateProduct(
        restaurantId, 
        productData.produto_id, 
        updatedProductData
      );
      ...
      if (onProductUpdated) {
        onProductUpdated(produtoAtualizado);
      }
      ...
      <View style={styles.overlay}>
        ...
        <View style={styles.modalContainer}>
          ...
          >
            ...
            >
              {/* Título */}
              <Text style={styles.title}>Editar informações do produto:</Text>
